...
import { AgentUpdate, ModelId } from "../types";
...
export function useAgent() {
  ...
  const submit = useCallback(async (overrideText?: string, contextScreenshot?: string, overrideMode?: string) => {
    ...
    const currentVoiceMode = storeState.voiceMode;
    const provider = storeState.selectedProvider;
    const customBaseUrl = storeState.customBaseUrl;
    const customModel = storeState.customModel;

    // determine actual model to use
    let modelToUse: ModelId = selectedModel;
    if (provider === "custom" && customModel) {
      modelToUse = customModel as ModelId;
    }

    try {
      console.log("[useAgent] invoking run_agent:", { text: text.slice(0, 50), model: modelToUse, mode, voiceMode: currentVoiceMode, provider, conversationId });
      await invoke("run_agent", { 
        ...
        history, 
        contextScreenshot: contextScreenshot ?? null, 
        conversationId,
        provider,

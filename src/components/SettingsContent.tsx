...
const API_PROVIDERS: { id: ApiProvider; label: string; url: string }[] = [
      ...
      <section>
        ...
        <div className="rounded-xl bg-white/[0.03] border border-white/5 p-4 space-y-3">
          ...
          <div className="flex items-center justify-between">
            <span className="text-[13px] text-white/90">Provider</span>
            <select
          ...
          {/* Model selection */}
          <div className="flex items-center justify-between">
            <span className="text-[13px] text-white/90">Model</span>
            <select
              value={selectedProvider === "custom" ? (customModel || selectedModel) : selectedModel}
              onChange={(e) => {
              ...
              className="px-2 py-1.5 text-[12px] bg-white/5 border border-white/10 rounded-md text-white/90 focus:outline-none focus:border-white/30 cursor-pointer"
            >
              {selectedProvider === "anthropic" && ANTHROPIC_MODELS.map((model) => (
                <option key={model.id} value={model.id} className="bg-zinc-900">
                  {model.label}
                </option>
              ))}
              {selectedProvider === "openai" && OPENAI_MODELS.map((model) => (
                <option key={model.id} value={model.id} className="bg-zinc-900">
                  {model.label}
                </option>
              ))}
              {selectedProvider === "custom" && (
                <>
                  ...
                  <option value="gpt-4-turbo" className="bg-zinc-900">GPT-4 Turbo</option>
                  <option value="claude-sonnet-4-5" className="bg-zinc-900">Claude Sonnet</option>
                  <option value="claude-haiku-4-5-20251001" className="bg-zinc-900">Claude Haiku</option>
                  <option value="llama-3.1-70b" className="bg-zinc-900">Llama 3.1 70B</option>
                  <option value="mistral-large" className="bg-zinc-900">Mistral Large</option>
                </>
              )}
            </select>
          </div>
          ...
          {selectedProvider === "custom" && (
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-[13px] text-white/90">Base URL</span>
                <input
                  type="text"
                  value={customBaseUrl}
                  onChange={(e) => setCustomBaseUrl(e.target.value)}
                  placeholder="http://localhost:11434/v1/chat/completions"
                  className="w-[200px] px-2 py-1.5 text-[11px] bg-white/5 border border-white/10 rounded-md text-white/90 placeholder-white/30 focus:outline-none focus:border-white/30 font-mono"
                />
              </div>
              <p className="text-[10px] text-white/40">
                Examples: Ollama (http://localhost:11434/v1/chat/completions), 
          ...
          <div className="pt-2 border-t border-white/5">
            <p className="text-[10px] text-white/40">
              {selectedProvider === "anthropic" && "Set your ANTHROPIC_API_KEY in .env"}
              {selectedProvider === "openai" && "Set your OPENAI_API_KEY in .env"}
              {selectedProvider === "custom" && "Set your API key based on your provider (OPENAI_API_KEY or ANTHROPIC_API_KEY)"}
            </p>
          </div>
        </div>
      </section>

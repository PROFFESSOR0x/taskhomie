...
async fn run_agent(
    ...
    println!("[taskhomie] run_agent called with: {} (model: {}, mode: {:?}, voice: {}, provider: {}, custom_url: {:?}, history: {} msgs, screenshot: {}, conv: {:?})",
        instructions, model, mode, voice, provider_str, custom_base_url, history.len(), context_screenshot.is_some(), conversation_id);
...

    {
        ...
        if !agent_guard.has_api_key() {
            return Err("No API key set. Please add ANTHROPIC_API_KEY or OPENAI_API_KEY to .env".to_string());
        }
        
        // Set provider and custom URL
        let api_provider = match provider_str.as_str() {
            "openai" => agent::ApiProvider::OpenAi,
            "custom" => agent::ApiProvider::Custom,
            _ => agent::ApiProvider::Anthropic,
        };
        agent_guard.set_api_provider(api_provider);
        agent_guard.set_custom_base_url(custom_base_url);
    }
...
fn main() {
    ...
    builder
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            permissions::get_all_api_keys_status,
